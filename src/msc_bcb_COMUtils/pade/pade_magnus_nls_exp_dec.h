...
struct pade_magnus_nls_t {
	...
	pade_magnus_nls_t(size_t _N, float_t _g = 0, complex_t _alpha = complex_t())
		:X_nls(_N), N(_N),
		g(_g), alpha(_alpha), alpha_prev(_alpha), omagnus(0), ncorr(0),w(1){
	...
	void set_magnus_NL() {
		...
		auto dec = 2*alpha_nl.real();

		auto magnus = [&](auto k, auto fxp) {

			auto rr0 = a * abs2(fx)+b * (abs2(fxm) + abs2(fxp));
			//auto rr0 = abs2(fx);
			auto  vm = g * abs2(fx);
			...
			if (_abs(vm) > 0) {
				...
				auto rr1 = - 2*(mul_imag_c(fx, fxm + fxp));
				auto G = rr1 / rr0;
				/*
				auto fdx = complex64_t(fx.real(), fx.imag());
				auto fdxp = complex64_t(fxp.real(), fxp.imag());
				auto fdxm = complex64_t(fxm.real(), fxm.imag());
				auto G =- 2*sin_delta_c(fdxp, fdxm, fdx);
				*/
				auto D = texp_1_0<double>((G+dec)*dt);
				vm *= D;
				//vm += g*dt/2.0* rr1;
			}
...
struct pade_magnus_nls_step_t:pade_magnus_nls_t<_COMPLEX, _FLOAT,_MEMPOOL> {
	...
	typedef typename pade_magnus_nls_step_t self_t;
   ...
   long	step(int pp = 0) {
	...
	static void nls_data_update(const char cmd, pade_magnus_nls_step_t& nls, nls_data_t* nls_data) {
