...
//LONG_PTR l= InterlockedCompareExchange(&m_val,m_val,m_val);
	LONG_PTR l=(LONG_PTR) InterlockedCompareExchangePointer((void**)&m_val,0,0);
   return l;
...
inline void*  proc_ptr()
	...
	LRESULT send(UINT umsg, WPARAM wparam , LPARAM lparam)
	{
		return SendMessage(m_hwin,umsg,wparam,lparam);
	}
};
