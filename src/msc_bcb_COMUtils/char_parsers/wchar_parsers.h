...
inline v_buf<char>&  unicode_to_char_buf(wchar_t* pw,int len=-1,unsigned int CP_XX=CP_UTF8,v_buf<char>& buf=v_buf<char>())
{

	//buf.clear();

	cleaner_t<v_buf<char> > cleaner(&buf);
	...
	if(pw)
	{
		...
		if(len<0)
			len=wcslen(pw)+1;
		int cbz=WideCharToMultiByte(CP_XX,0,pw,len,0,0,0,0);	


		if(cbz)
		{
			buf.resize(cbz-1);
			if(cbz==WideCharToMultiByte(CP_XX,0,pw,len,buf,cbz,0,0))
				cleaner.detach() ;
		} 
...
inline v_buf<wchar_t>& char_to_unicode_buf(char* s_utf,int len=-1,unsigned int CP_XX=CP_UTF8,v_buf<wchar_t>& buf=v_buf<wchar_t>())
{
	...
	cleaner_t<v_buf<wchar_t> > cleaner(&buf);
	...
	if(s_utf)
	{
		...
		int cbz=MultiByteToWideChar(CP_XX,0,s_utf,len,0,0);

		if(cbz)
		{
			buf.resize(cbz-1);	
			if(cbz==MultiByteToWideChar(CP_XX,0,s_utf,len,buf,cbz))
				cleaner.detach();
...
struct char_mutator
{
	...
	inline int char_count(bool with_tail=false)
	{
		get_pc(); 
		return buf.count(with_tail);
	}

	inline int wchar_count(bool with_tail=false)
	{
		get_pwc(); 
		return wbuf.count(with_tail);
	}
...
inline 	tstring& get_stream_cache(const tstring&s= tstring()) {
		_string_cache.push_front(s);
		return _string_cache.front();
	}

inline 	CH* get_cache(const tstring&s = tstring()) {
	return (CH*)get_stream_cache(s).c_str();
}

    

	std::list<tstring> _string_cache;
};
...
struct argzz_setter
{
	...
	void   operator ()(O* po,V v)
	{
		...
		argv_zzbase<CH, argzz_setter<CH> >* pzo = (argv_zzbase<CH, argzz_setter<CH> >*)po;
		
		std::vector<CH*>& ppvect = pzo->ppvect;

		O::tstringstream stream;

		stream << name << sequ << v << CH(0);
		...
		//O::tstring& s = pzo->get_string_cache(stream.str());

        CH*& pline = pzo->ppvect[ind];

		pline = pzo->get_cache(stream.str());
...
struct argv_zz:argv_zzbase<CH,Setter,LIST>
{
	...
	argv_zz(int cb,const CH* _ppargv)
	{
	...
	const  CH* init(int cb,const CH* _ppargv=0)
	{
		CH* p=0;
	...
	inline argv_zz& clear_cache() {

		 return convert(*this, CP_UTF8);
	}
...
struct argv_zzs:argv_zz<CH,Setter,LIST>
{
	...
	argv_zzs(int cb,const CH* _ppargv=0):argv_zz(cb,_ppargv)
	{
	...
	template <class Other>
	inline argv_zzs& update(Other& a) {

		v_buf<CH> buf;
		for (int n = 0; n < a.argc; n++) {
			CH* name = (CH*)a.names(n, buf);
			//(*this)[name] =n+11;
			(*this)[name] = (CH*)a[name];
		}
...
struct s_parser_t
{
	...
	{
		return undecorate(undecorate(trim(str),'\'','\''),'\"','\"');
	}


	CH* rq(const CH* str) {

		return undecorateQQ(str);
	}
