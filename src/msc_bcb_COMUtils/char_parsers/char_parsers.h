...
struct v_buf
{
	...
	inline 	static	DWORD _toLowerStr(char* buf, DWORD len) {

		return CharLowerBuffA(buf, len);
	}

	inline 	static	DWORD _toLowerStr(wchar_t* buf, DWORD len) {

		return CharLowerBuffW(buf, len);
	}
	...
	inline size_t count(bool with_tail = false)
	{
		int c = v.size();
		if (with_tail) return (c > tail) ? (c) : 0;
		else 	return (c > tail) ? (c - tail) : 0;
	}
...
inline    int safe_len_zerozero(const CH* pb, bool fwith_last_zero = false)
{
	...
	size_t cb = (size_t(p) - size_t(pb)) / sizeof(CH);
...
template <typename CH>
int str_bin(const CH* str, unsigned long long& val)
{
	CH* p = (CH*)str;
	int cb = safe_len(p);
...
template <typename CH>
int str_hex(const CH* str, unsigned long long& val)
{
	CH* p = (CH*)str;
	int cb = safe_len(p);
	const CH * digs = dflt_v<CH>::digits();
...
inline char* find_iter(const char* ps, int len, char* pb, char* pe)
{
	...
	while (p < pe) {
		if (*ps == *p)
		{
			size_t n = min(len, size_t(pe) - size_t(p));
...
template <class region>
inline char* __get_header_cr(char* hn, region r, int* pcb)
{
	int cb = strlen(hn);
	char* p = r.ptr;
	if (!p) return 0;
	...
	for (p; p < pe; ++p)
	{
		if (strnicmp(hn, p, cb) == 0)
		{
			int &c = *pcb;
			char *pv = p + cb, *pc = pv;
			while (((*pc != 0x0D) && (*pc != 0x0A)) && (pc < pe)) ++pc;
...
#define _GET_HEADER_CR(a,r,pi) __get_header_cr("\n" #a ": ",r,pi)
