...
#define _FAST_DIV(a,b) _fast2_div(a,b)


//#define DBG_000_CR

namespace trimatrix::parallel {
	//namespace parallel {
		...
		template < class _F, int M = 1, class _MEMPOOL = utils::mem_pool_t<>,class _PAR_FOR= fake_parfor_t,bool frep=true,  template <class, class, bool> class _SOLVER = thomas_cycle_t >
		struct trimatrix_cycle_reduction_t :trimatrix_base_t<_F, M, _MEMPOOL> {
			...
			template <class XX>
			void disassemble_pp(bool pp, size_t grain, XX& target, XX& even, XX& odd) {

				auto N = this->N;
				auto& xe = even.xx;
				auto& xo = odd.xx;
				parfor(pp,0, N, [&](auto b, auto e) {
			...
			template <class XX>
			inline  void assemble_pp(bool pp, size_t grain, XX& even, XX& odd, XX& sourse) {
			...
			void init() {
				if (level > 0) {
					...
					if (N2) {
						...
						this->pool.alloc(N2, &gg_even);
						this->pool.alloc(N2, &dd_even);

						this->pool.alloc(N2, &gg_odd);
						this->pool.alloc(N2, &dd_odd);
						
						//this->even.reset(new trimatrix_cycle_reduction_t(N2, l2));
						//this->odd.reset(new trimatrix_cycle_reduction_t(N2, l2));
						this->even = std::make_unique<trimatrix_cycle_reduction_t>(N2, l2);
						this->odd = std::make_unique<trimatrix_cycle_reduction_t>(N2, l2);
...
#define NDBG 8
				...
				auto make_row = [&](auto n, auto n2, auto n2m, auto n2p, auto n2pp) {
					...
					if constexpr (fodd) {
						...
						if constexpr (nof) {
							...
							d = _FAST_DIV(b[n2p], az + a[n2]);
							g = _FAST_DIV(c[n2p], az + a[n2pp]);
						...
						if constexpr (nof) {
							...
							d = _FAST_DIV(c[n2], az + a[n2p]);
							g = _FAST_DIV(b[n2], az + a[n2m]);
			...
			long factorize_solve(bool pp, XX& xx, size_t grain = 1, f_t az = f_t()) {
				...
				disassemble_pp(pp,grain, xx, even->yy, odd->yy);
				...
				assemble_pp(pp, grain, even->yy, odd->yy, xx);

#ifdef DBG_000_CR
				bN_t<f_t, NDBG> bbb;bN_t<f_t, NDBG/2> bbb2;
				auto pxx = pN_t<NDBG>::ptr(xx.xx[0]);
				auto pxxe = pN_t<NDBG/2>::ptr(even->yy.xx[0]);
				auto pxxo = pN_t<NDBG/2>::ptr(odd->yy.xx[0]);
#endif
				return 0;
			}
			...
			long solve(bool pp, size_t grain = 1, f_t az = f_t()) {
				...
				return factorize_solve<frep>(pp, this->yy, grain, az);

			}

			template <class XX>
			long solve(bool pp, XX& xx, size_t grain = 1, f_t az = f_t()) {
				return factorize_solve<frep>(pp, xx, grain, az);
			}

			
			long solve(bool pp, f_t** yy, size_t grain = 1, f_t az = f_t()) {

				xx_t<f_t, M> xx;
				return factorize_solve<frep>(pp, attach_xx(xx,yy), grain, az);
			}
