...
template <int _capacity=1*64*MB,bool freemarshall=1>
...
#pragma pack(pop)
  ...
  HRESULT make_post_filters(BITMAPFILEHEADER*& pfh,byte_t*& pbits){
   ...
   virtual HRESULT GetAdditionDataPtrSize(byte_t** ppbits,long* psize=0)
   {
	   HRESULT hr;
      if(!ppbits) return E_POINTER;
	  byte_t* ptr =bmpmap.ptr;
	  long offset=_capacity/2;
		  //  bmpmap.image_file_size_b()+sizeof(BMPMap::frame_info_base_t);	 
	  *ppbits=ptr+offset;
	  long cb=_capacity-offset;
	  if(cb<=0) return E_OUTOFMEMORY;
	  if(psize) *psize=cb;
	  return S_OK;
   }

	virtual HRESULT PushFrame(void* pbmpimage,const byte_t* pcbits=0, LARGE_INTEGER* pTimeStamp=0)
	{
		...
		*/
          ...
          BMPMap::frame_info_base_t* pinfo=0;
		 ...
		 if(pTimeStamp)
		 {
			 pinfo=(BMPMap::frame_info_base_t*)(ptr-sizeof(BMPMap::frame_info_base_t));
			 pinfo->TimeStamp=*pTimeStamp;
		 }
	...
	inline		frame_data_t* get_next_and_lock(int mode,BMPMap::frame_info_t** ppinfo=0)
	{
...
inline	HRESULT unlock_by_ptr(byte_t* ptr)
	...
	virtual HRESULT GetFrame(BITMAPFILEHEADER** ppbmpimage,int* pcbimage=0, LARGE_INTEGER* pTimeStamp=0){
...
//			     pbfh->bfOffBits=cbh;
...

			   {
				   ...
				   BMPMap::frame_info_base_t* pinfo=0;
				   char*  p= (char*)lockptr.get();
				   if(pTimeStamp)
				   {
                       pinfo=(BMPMap::frame_info_base_t*)(p-sizeof(BMPMap::frame_info_base_t));
					  *pTimeStamp= pinfo->TimeStamp;
				   }
				   memcpy(pbits_s,p,cbb);
			   }
                   hr=S_OK;
