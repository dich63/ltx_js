...
struct AviSynthDecayGreenCache_base:public avs_base<Version>
{
	typedef unsigned int uint_t;

	typedef typename AviSynthDecayGreenCache_base<Version> base_t;
     ...
     enum    
	{
		...
		std::vector<DWORD> vbuf;
		DWORD * pdwbuf;
		long offset_dwbuf;
		BYTE* pframe_ptr;


		AviSynthDecayGreenCache_base():handled_count(-1){};
	 AviSynthDecayGreenCache_base(PClip _clip,AVSValue _Filter,AVSValue _Params)
		 ...
		 
	 {
		 ...
		 alpha_2=1;
		 frame_getter=new CurrentGetter<AviSynthDecayGreenCache_base>(this);
		 RECT r={0,0,0,0};
		 rc=rc_old=r;
         mt_handler.po=this;
		 mt_handler.mode=0;
...
inline DWORD* get_cache_ptr(BYTE* pdest)
   {
	        int off=int(pdest)-int(pframe_ptr);
          return  (DWORD*)(pdwbuf+off); 
   }
	 ...
	 void copy(int n,IScriptEnvironment* env=0)
	 {
		 ...
		 for(int y=0;y<vi.height;y++)
		 {
			 ...
			 DWORD* pdwdest=get_cache_ptr(pdest);
			 for(int x=0;x<row_size;x++)
			 {
				 const BYTE b=psrc[x];
				 pdest[x]=b;
				 pdwdest[x]=DWORD(b)<<alpha_denom_dig;
			 }
	...
	inline void init_once(int n,IScriptEnvironment* env)
	{
		...
		row_size = frame->GetRowSize();
		 ...
		 vbuf.resize(vi.height*pitch*4);
		 pdwbuf=&vbuf[0];

		 pframe_ptr=frame->GetWritePtr();
		 offset_dwbuf=int(pdwbuf)-int(pframe_ptr);
...
avs_fast_call_f void blend_pt(const BYTE* pline_src,BYTE* pline_dest,uint_t a,uint_t b,DWORD* pdwdest)
{
	uint_t v;
...
avs_fast_call	void blend_line00(BYTE *pline_dest,const BYTE *pline_src)
	{
		...
		DWORD* pdwdest=get_cache_ptr(pline_dest);

		for(;pline_dest<pend;++pline_dest,++pline_src,++pdwdest)
	   {
		   /*
          uint_t v;
		    v= (b*(*pdwdest))>>alpha_denom_dig;
		  ...
		  *pline_dest=v>>alpha_denom_dig;
		  ...
		  ++pdwdest;
		  */
            blend_pt(pline_src,pline_dest,a,b,pdwdest);
		...
		for(;pline_dest<pend;++pline_dest,++pline_src,++pdwdest)
		{
			if(in_interval(pleft,pline_dest,pright))
				...
				if(in_interval(pleft_old,pline_dest,pright_old))
				{
				  ...
				  //*pline_dest=v>>alpha_denom_dig;
					blend_pt(pline_src,pline_dest,a2,b2,pdwdest);
...
inline	 RECT get_fix_rect(int wi,int hi,RECT rin)
 ...
 /*
 template <class Version=version>
 struct AviSynthTimeCoarsenCache:AviSynthDecayGreenCache_base<Version>
 {
	 int FrameModulo;
	 int Shift;

	 AviSynthTimeCoarsenCache(PClip _clip,AVSValue Fm,AVSValue _Prm):AviSynthDecayGreenCache_base<Version>(_clip,Fm,_Prm)
	 {
	 ...
	 static AVSValue __cdecl Create(AVSValue args, void*, IScriptEnvironment* env) {
		 ...
		 return new AviSynthTimeCoarsenCache(child,args[1],args[2]);
	 }
 };
*/

 template <class Version=version>
 struct AviSynthDecayGreenCache_simple:AviSynthDecayGreenCache_base<Version>
 {
	 int keyF;
	 bool fsub;
	 CStopwatch csg;
	 
	 AviSynthDecayGreenCache_simple(PClip _clip,AVSValue _Filter,AVSValue _Prm,AVSValue afsub):AviSynthDecayGreenCache_base<Version>(_clip,_Filter,_Prm)
	 {
		 double f=Filter.AsFloat(1);
		 fsub=afsub.AsFloat(0);
	 ...
	 static AVSValue __cdecl Create(AVSValue args, void*, IScriptEnvironment* env) {
           ...
           PClip child=args[0].AsClip();   
		 return new AviSynthDecayGreenCache_simple(child,args[1],args[2],args[3]);
	 }

 };

 template <class Version=version>
 struct AviSynthDecayGreenCache_callback:AviSynthDecayGreenCache_base<Version>
 {
	 charbuf func_name;
	 ;
	 int keyF;
	 ...
	 AviSynthDecayGreenCache_callback(PClip _clip,AVSValue _Filter,AVSValue _Prm,AVSValue handler_key):AviSynthDecayGreenCache_base<Version>(_clip,_Filter,_Prm)
	 {
...
~AviSynthDecayGreenCache_callback()
{
...
//Params
	...
	//AVSValue res=invoke(env,func_name,8,child,n,frame_getter,handler,rc_old.left,rc_old.top,rc_old.right,rc_old.bottom);
		...
		keyF=get_value("keyF",0).AsFloat(0);
		set_value("keyF",0);
   ...
   return res;
	...
	if(1)
	{
	 ...
	 static AVSValue __cdecl Create(AVSValue args, void*, IScriptEnvironment* env) {
		 ...
		 return new AviSynthDecayGreenCache_callback(args[0].AsClip(),args[1],args[2],args[3]);
	 }
