...
struct shared_image_loader_t
{
	...
	shared_image_loader_t():hr(E_POINTER){};

	shared_image_loader_t(const wchar_t* key,const wchar_t* pfilefmt):shared_image(key),filefmt(pfilefmt) 
	{
		hr=shared_image.hr;
	}
     
	shared_image_loader_t(ISharedImage* si,const wchar_t* pfilefmt)
	{
           init(si,pfilefmt);
	}


    HRESULT init(ISharedImage* si,const wchar_t* pfilefmt) 
	{ 
		if( pfilefmt&&si)
		{		
		 filefmt=pfilefmt;
	...
	int load(int nframe)
	{
		...
		FILE* hf=_wfopen(fnb,L"rb");
		if(hf)
		{
			int cbimage=_file_size(hf);
			buffer.resize(cbimage);
			char* p=buffer;
			BITMAPFILEHEADER* pbfh=(BITMAPFILEHEADER*)p;
		     cb=fread(pbfh,1,cbimage,hf);
			
			if(FAILED(hr=shared_image->PushFrame(pbfh)))
				cb=0;
			fclose(hf);  
			return cb;
		}
