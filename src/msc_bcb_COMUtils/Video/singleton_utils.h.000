...
inline HRESULT file_dir_delete(const WCHAR* name,bool f=false)
{
	SHFILEOPSTRUCTW shfo={0,FO_DELETE,name,NULL,FOF_NOERRORUI|FOF_NOCONFIRMATION|FOF_SILENT};
...
/*
		...
		bool              bDeleteSubdirectories = true)
	{
	 ...
	 if(safe_len(name))	return !del_t::DeleteDirectory(name,f);
	 else return E_FAIL;
*/
}
...
struct mutex_cs_t
{
	...
	mutex_cs_t(unsigned int sc=0){
		...
		InitializeCriticalSectionAndSpinCount(&m_cs,sc);
	}
	~mutex_cs_t()
	{
...
struct kobj_t
{
   ...
   {
	   return WaitForSingleObject(hko,tiot);
   };
...
{
	event_local_t(bool fmanual=true,bool fsignaled=0):event_ref_t(CreateEvent(0,fmanual,fsignaled,0)){};
	~event_local_t(){ CloseHandle(hko);}
};
...
struct console_shared_data_t
{
	...
	console_shared_data_t():sha(_prefix(),0,Size){};
	inline CH* str()
	{
...
struct _pool
{
    ...
    struct ko_t
	{
       T* pt;
	   HANDLE hkr;
	   ko_t(HANDLE h,T* p):pt(p),hkr(h){};
	   ~ko_t() {     UnregisterWait(hkr);  }
...
inline	void clear()
  ...
  {
	  if(ht)
	     return _pool<T,true>(p).call_in_thread(ht);
	  else  return 0;
  }
...
inline HRESULT elevate_run(const wchar_t* ApplicationName,const wchar_t* CommandLine,PROCESS_INFORMATION* ppi=NULL,DWORD nShow = SW_SHOWNORMAL,bool frunas=true)
{
	...
	Shex.nShow = nShow;
		...
		OSVERSIONINFO os=OSVERSIONINFO();
		os.dwOSVersionInfoSize = sizeof(OSVERSIONINFO);
		if(!GetVersionEx(&os)) return -1;
...
inline static  wchar_t* GMKB( double v ,bufT<wchar_t>& buf=bufT<wchar_t>())
{
	...
	const double kB=1024,mB=kB*kB,gB=mB*kB;
	const  wchar_t* prfx[]={L"B",L"KB",L"MB",L"GB"};
