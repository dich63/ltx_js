...
struct  _proc_stub_N
{
	...
	inline _proc_stub_N* init_N(void *_proc,void**params)
	{
		//va_list ptr;
		//va_start(ptr,fmt);
		paddr=_proc;    
...
template <int N>
LONG_PTR remote_call_kernel32(HANDLE hp,char* name,...) 
{
 class  rp_Proc :public _proc_stub_N<N,1>
{
 public:
	rp_Proc(char* procname,void** params)
	{
          
          void* pproc=GetProcAddress(GetModuleHandle(TEXT("Kernel32")),procname);
           init_N(pproc,params);
	};
};

 void** params=((void**)&name)+1;
 rp_Proc rp(name,params);
 remote_block rb(hp,sizeof(rp_Proc),&rp);
 return rb.call();
...
class  rp_DuplicateHandle:public _proc_stub_N<7,1>
{
	//  public:
	//friend  int testR(DWORD pid);
	//friend 	BSTR RemoteGetEnvironmentVariable(wchar_t* name,DWORD pid);
	//friend  int DuplicateHandle(,DWORD pid);
	//
protected:
    HANDLE m_ht;

	
	rp_DuplicateHandle(HANDLE hpsrc,HANDLE hsrc,HANDLE htp,DWORD da,BOOL binh,DWORD dwopt)
	{
     	void* pproc=GetProcAddress(GetModuleHandle(TEXT("Kernel32")), "DuplicateHandle");   
		init(pproc,hpsrc,hsrc,htp,&m_ht,da,binh,dwopt);
	}
