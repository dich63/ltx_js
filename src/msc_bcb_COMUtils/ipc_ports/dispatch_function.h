...
#include "dispatch_by_name_aggregator.h"
...
namespace ipc_utils{
...

	{
		...
		inline	HRESULT impl_Invoke(DISPID id,REFIID riid,LCID lsid,WORD flags,DISPPARAMS*dp,VARIANT*res,EXCEPINFO*pexi, UINT* pu){
			...
			return base_callsback_t::impl_Invoke(id,riid,lsid,flags,dph,res,pexi,pu,this);
		}
		...
		calback_impl_t(void* _pcalback=0,void* _context=0,void* _on_exit=0,DISPID id=DISPID_VALUE,DWORD _flagmask=DISPATCH_PUTSF)
			:base_callsback_dispatch_t(_pcalback,_context,id,_flagmask),on_exit(_on_exit),freverse(0){
				...
				_on_exit&&ipc_utils::module_t<>::DLL_AddRef(_on_exit);
				_pcalback&&ipc_utils::module_t<>::DLL_AddRef(_pcalback);
		};
		...
		~calback_impl_t()
		{
			...
			on_exit&&ipc_utils::module_t<>::DLL_Release(on_exit);
		}
		...
		,base_callsback_dispatch_t<calback_aggregator_t<IDispatchX>,IFakeUnknown_t>
	{
		...
		inline bool inner_QI(IUnknown* punkOuter,REFIID riid,void** ppObj,HRESULT& hr)
		{
			if(riid==GUID_NULL)
			{
				*ppObj=context;
			...
			else return __super::inner_QI(punkOuter,riid,ppObj,hr);
		}
		...
		struct cc_context_t:base_aggregator_t<cc_context_t,i_context_with_cache>
		{
			typedef  std::list< variant_t > varlist_t;

			cc_context_t(void* pc,iaggregator_container* container)
				:base_aggregator_t<cc_context_t,i_context_with_cache>(container)
				,pcontext(pc){}	
			...
			{              
				...
				bool f;
				f  =(riid==__uuidof(i_context))||(riid==__uuidof(i_context_with_cache));
				if(f)
				{
			...
			virtual HRESULT STDMETHODCALLTYPE PushCacheValue(VARIANT value)
			{
				locker_t<mutex_cs_t> lock( mutex);
				varlist.push_front(variant_t(value));				
				return S_OK;
			}
		...
		struct creator_calback_aggregator_t
		{
			...
			struct closer_t
			{
				...
				bool  f;
				apartment_t* papartment;
				closer_t(void* _pcalback,void* _on_exit,apartment_t* _papartment,void *context,bool fress):pcalback(_pcalback),pon_exit(_on_exit),papartment(_papartment),pcontext(context),f(fress)
				{
					pon_exit&&ipc_utils::module_t<>::DLL_AddRef(pon_exit);
					pcalback&&ipc_utils::module_t<>::DLL_AddRef(pcalback);
				}
				~closer_t()
				{
					if(f&&pon_exit)
						 safe_caller_t<>::safecall(pon_exit,pcontext);

					if(papartment) apartment_t::on_exit(papartment);
					pcalback&&ipc_utils::module_t<>::DLL_Release(pcalback);
					pon_exit&&ipc_utils::module_t<>::DLL_Release(pon_exit);
			...
			HRESULT operator()(DWORD t) 
			{
				...
				typedef dispatch_by_name_aggregator_t<com_apartments::process_com_t> dbn_aggregator_t;

				WORD flags=0xFFFF&exflags;
				WORD fapart=exflags>>16;
				COMInitF_t ci;
				aggregator_container_helper  container(new closer_t(pcalback,on_exit,papartment,context,fapart&FLAG_CBD_DBN),NULL,&closer_t::on_exit);
				fstart=1;
				if((fapart&FLAG_CBD_FTM)||(fapart&FLAG_CBD_GIT)) 
					container.add_FMT();
				//if(fapart&FLAG_CBD_EC) container.add_EC();
				smart_ptr_t<IUnknown> aggr,aggr_cntx;
				...
				if(fapart&FLAG_CBD_GIT){
					...
					//calback_impl_t<>* p=new calback_impl_t<>(pcalback,context,NULL,DISPID_VALUE,exflags);
					p->freverse=frev; 
					...
					if(fapart&FLAG_CBD_DBN)
					{

						IUnknown* punk=container.container;

						dbn_aggregator_t *p=new dbn_aggregator_t(punk,pcalback,context);
                        aggr=aggregator_helper_creator_t<dbn_aggregator_t>::New(p);
				 ...
				 aggr_cntx=aggregator_helper_creator_t<cc_context_t>::New(new cc_context_t(context,container));


				if(fapart&FLAG_CBD_EC) 
					mta_singleton_t::add_external_connector(container);

				hr=container.add(__uuidof(IDispatch),aggr).add(GUID_NULL,aggr)
					.add(__uuidof(i_context),aggr_cntx).add(__uuidof( i_context_with_cache),aggr_cntx).unwrap(&dwCookie);
				return hr;
			}
