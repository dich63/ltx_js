	...
	namespace com_apartments
	{
		...
		struct apartment_t: _non_copyable_
		{
			...
			inline void proc()
			{
				...
				BOOL fem=set_thread_error_mode(SEM_NOOPENFILEERRORBOX|SEM_FAILCRITICALERRORS);
			...
			//
				...
				//if(am==COINIT_APARTMENTTHREADED) 
				//	initer.hr=E_POINTER;
				HRESULT hrr;
				hrr=CoEnableCallCancellation(0); 
				fem=set_thread_error_mode(SEM_NOOPENFILEERRORBOX|SEM_FAILCRITICALERRORS);
