...
*/
       ...
       HRESULT attach(DWORD tio=INFINITE)
	   {
		...
		if(SOCKET_ERROR==WSAEventSelect(s0,hevent,FD_CLOSE))//|FD_READ
		   hr=WSAGetLastError();
   ...
   static void  __stdcall io_completition(HRESULT hr,ULONG_PTR bytetransfer,io_overlapped_t* povl)
	...
	bool recv_op()
	{
	   ...
	   struct asyn_channel_t
	   {
		   ...
		   HRESULT operator()()
		   {
		    console().printf(M_BC|0x0e,"channels=%s |valence|=%d - connected \n",sp->name.c_str(),abs(sp->charge));
			 HRESULT hr=hss_wbs_frame_from_to((void*)s0,(void*)s1,BUFDEFSZ_ASYN);
			 console().printf(M_BC|0x0A,"channels=%s |valence|=%d - disconnected\n",sp->name.c_str(),abs(sp->charge));
...
void  on_accept(HSS_INFO* ovl)
{
	...
	char ping[]="pingZ++";
	int cb0=strlen(ping)+1,cb;
    char pong[64]="?????????????????????????";

	if(0)
	{
		 v_buf<char> buf(24*1024*1024);
		 char* p=buf;
         hr=hss_read_wbs_frame(ovl,0,p,&(cb=buf.size_b()),&wbs_frame);
	}

	//

	//hr=hss_write_wbs_frame(ovl,ping,&(cb=cb0),WSF_PIN|WSF_FIN,0);
	//hss_read_wbs_frame(void* hsinfo,int fnext,void* buf,int* pcb,wbs_frame_base_t* pwbs_frame)
	//hr=hss_read_wbs_frame(ovl,0,pong,&(cb=cb0),&wbs_frame);
