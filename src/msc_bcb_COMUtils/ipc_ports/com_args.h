...
struct args_base_t:IDispatch_base_t<T,IDispatchX,RefCounter>
{
	...
	args_base_t():raise_error(0),leftbound(0){};
	args_base_t(int c,VARIANT *pv,bool f_reverse=true,raise_error_proc_t re=0):argc(c),pargv(pv),freverse(f_reverse),raise_error(re),leftbound(0){};

	args_base_t(DISPPARAMS* pdp,bool f_reverse=true):argc(pdp->cArgs),pargv(pdp->rgvarg),freverse(f_reverse),raise_error(0),leftbound(0){};
	...
	inline HRESULT shift(VARIANT *pres=0)
	{
		locker_t<mutex_t> lock(mutex);
		HRESULT hr;
		if(argc>0)
		{
			...
			if(pres){
				...
				int i=(freverse)?argc-1:0;
				OLE_CHECK_hr(VariantCopyInd(pres,pargv+i));
	...
	inline HRESULT findname(LPOLESTR name,DISPID& dispid){
		...
		static  BEGIN_STRUCT_NAMES(pn)										
			...
			DISP_PAIR_IMPLC_LEN(item,id_item,4)
			DISP_PAIR_IMPLC_LEN(shift,id_shift,5)
			END_STRUCT_NAMES;
	...
	inline	HRESULT impl_Invoke(DISPID id,REFIID riid,LCID,WORD flags,DISPPARAMS*dp,VARIANT*res,EXCEPINFO*pexi, UINT*){
		...
		if(((id==DISPID_VALUE)||(id==id_item))&&(dp->cArgs>=1))
		{
			VARIANT vi={};
          if(dp->cArgs==1)
		  {
		...
		if(check(i))
		{
			...
			if(id==id_len)		   		   	   	
				...
				if(id==id_def)
					...
					{
						VARIANT vi={};
						if(FAILED(VariantChangeType(&vi,dp->rgvarg+dp->cArgs-1,0,VT_I4)))
							return hr;
						if(check(i=vi.intVal-leftbound))
						{
							if(freverse) i=(argc-1)-i;
							return hr=VariantCopyInd(res,pargv+i);
						}
...
struct args_stack_t:args_base_t<args_stack_t,RefCounterFake_t<1> > 
{
	args_stack_t(int c,VARIANT *pv,bool freverse=true)
		:args_base_t(c,pv,freverse){}
	args_stack_t(DISPPARAMS* pdp):args_base_t(pdp){};
} ;