...
#define CHECK_HANDLE(h) ((h!=NULL)&&(h!=INVALID_HANDLE_VALUE))
...
namespace ipc_utils {
	...
	struct module_helper_t
	{
		...
		module_helper_t(const wchar_t* modulename=0):pbase(0),hmap(0), hfile(INVALID_HANDLE_VALUE),hr(S_OK)
		{
			...
			if(modulename)
			{
				...
				HANDLE hm,hf=hfile=CreateFileW(modulename,GENERIC_READ,FILE_SHARE_READ,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);
    ...
    static	 HWND wnd_lite(char* winclass)
		...
		public:
			HRESULT hr;
			ClipboardText():hr(S_OK)
			{
				hwin=wnd_lite("Clip00");
				hr=S_OK;
				OLE_CHECK_VOID_hr_cond(OpenClipboard(hwin),GetLastError());
			};
			ClipboardText(bstr_t str):hr(S_OK)
			{
				hwin=wnd_lite("Clip00");
				OLE_CHECK_VOID_hr_cond(OpenClipboard(hwin),GetLastError());
				;
				OLE_CHECK_VOID_hr_cond(EmptyClipboard(),GetLastError());
				...
				if (cb) {
					...
					HANDLE hm = GlobalAlloc(GMEM_FIXED, cb<<1);
					StrCpyW((wchar_t*)hm, str);
					OLE_CHECK_VOID_hr_cond(SetClipboardData(CF_UNICODETEXT, hm), GetLastError());

					
					hm = GlobalAlloc(GMEM_FIXED, cb);
					StrCpyA((char*)hm, str);
					OLE_CHECK_VOID_hr_cond(SetClipboardData(CF_TEXT, hm), GetLastError());
				}
				loop(hwin);
...
*/
	...
	struct process_handle_t
	{
		...
		inline HRESULT terminate(unsigned int ec=0)
		{
			HANDLE h=get_handle(PROCESS_TERMINATE);
			if(h)
			{
				if(!TerminateProcess(h,ec))
					hr=GetLastError();
...
inline	SOCKET dup_socket(SOCKET s)
	...
	inline BOOL check_handle(HANDLE h)
	{
         DWORD dw;
		 return h&&GetHandleInformation(h,&dw);
	}

	inline BOOL safe_close_handle(HANDLE h)
	{
		BOOL f = check_handle(h);
		return  f&&::CloseHandle(h);
	...
	struct module_t
	{
		...
		inline static wbuf_t  module_path(void* p=((void*)-1),HRESULT* phr=0)
		{
			HRESULT hr;
			wbuf_t buf,buf2;
			wchar_t* pt=0;
			const void* pm0=((void*)-1);
			void* pm=(p!=pm0)?p:((void*)&DLL_AddRef);
			buf=module_name(pm, &hr);
		...
		inline static wbuf_t  module_name(void* p=((void*)-1),HRESULT* phr=0)
		{			
             wbuf_t buf;
			 HRESULT hr;
			 const void* pm0=((void*)-1);
			 void* pm=(p!=pm0)?p:((void*)&DLL_AddRef);
			 HMODULE h=__getHinstance(pm,false,&hr);
			 if(SUCCEEDED(hr))
			 {
	...
	struct file_seek_t {
		...
		LARGE_INTEGER ll;
		HRESULT hr;
		file_seek_t(HANDLE hf, LONGLONG pos=0L ) :hfile(hf), hr(S_OK) {
	...
	inline static HANDLE create_job(const wchar_t* name = NULL,DWORD flags= JOB_OBJECT_LIMIT_DIE_ON_UNHANDLED_EXCEPTION | JOB_OBJECT_LIMIT_BREAKAWAY_OK | JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE) {
		SECURITY_ATTRIBUTES _sa = {};
		HANDLE h;		
		h = ::CreateJobObjectW(&_sa, name);
		if (h) {
