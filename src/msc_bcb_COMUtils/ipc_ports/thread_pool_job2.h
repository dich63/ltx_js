...
struct thread_pool_job_t
{
	...
	typedef	struct job_packet_t {
		...
		typedef int(*packet_proc_t)(void* context,void* params,int n);
...
struct packet_base_t:thread_pool_job_t<>::job_packet_t {
	...
	static int packet_proc_s(void* context, void* params, int n) {
		return static_cast<T*>(context)->make((P)params);
