		...
		namespace script_engine
		{
				...
				:class_factory_base_aggregator_t<asyn_factory_t>
			{
				...
				struct asyn_disp_t:IDispatch_impl_t<asyn_disp_t>					
				{
					...
					inline	HRESULT impl_Invoke(DISPID id,REFIID riid,LCID,WORD flags,DISPPARAMS*dp,VARIANT*res,EXCEPINFO*pexi, UINT*){
						...
						try
						{
							...
							VARIANT hold =(VARIANT) arguments[0];
							variant_cast_t<VT_BSTR> vtext(hold,VARIANT_NOVALUEPROP);
							bool fscript=vtext;
							if(fscript)
							{
							...
							OLE_CHECK_hr(asyn_object_t::CreateInstance(funct,eventobj,tio,&asynobj.p));
				...
				inline HRESULT impl_CreateInstanceEx(	IUnknown * pUnkOuter,LPOLESTR pszDisplayName,IStream* ,REFIID riid,void ** ppvObject,IBindCtx* pbc)	
				{
					...
					int tio=mp.bind_args(L"tio").def<int>(0);

					smart_ptr_t<iaggregator_helper> asyn=aggregator_helper_creator_t<asyn_disp_t>::New(new asyn_disp_t(tio));
				...
				:simple_factory_t<file_search_factory_t>					
			{
				struct fs_disp_t:IDispatch_impl_t<fs_disp_t>					
				{
					...
					inline	HRESULT impl_Invoke(DISPID id,REFIID riid,LCID,WORD flags,DISPPARAMS*dp,VARIANT*res,EXCEPINFO*pexi, UINT*){
						...
						try
						{
							...
							if(fc==512)
							{
								  bstr_t search_path2=arguments[3].def(bstr_t());
								  OLE_CHECK_hr(mon.cmdline_parse(&fulname,fn,search_path,search_path2));
                                  
							}
							else
							{	
								bstr_t ext_def=arguments[3].def(bstr_t());
								OLE_CHECK_hr(mon.expand_file_path(fn,&fulname,search_path,fc,ext_def));
							}
				...
				inline HRESULT create_iaggregator(aggregator_container_helper_t& container,LPOLESTR pszDisplayName,IStream* ,GUID& uiid,smart_ptr_t<iaggregator_helper>& fs_disp,IBindCtx * pbc )
				{
				...
				:simple_factory_t<external_connect_locker_factory_t>					
			{
				...
				struct external_connect_locker_t:IDispatch_impl_t<external_connect_locker_t>					
				{
					...
					inline HRESULT findname(LPOLESTR name,DISPID& dispid){
						...
						static  BEGIN_STRUCT_NAMES(pn)										
							...
							DISP_PAIR_IMPLC(wait, id_wait)
							DISP_PAIR_IMPLC(signal, id_signal)
							DISP_PAIR_IMPLC(handle, id_handle)
							DISP_PAIR_IMPLC(msgwait,id_msgwait)							
							DISP_PAIR_IMPLC(refcount, id_refcount)
							END_STRUCT_NAMES;
						...
						if(id==DISPID_VALUE)
						{
							...
							{ 
								...
								if( pid=args[1].def<int>(0)){
									...
									if(ph.get_handle())
									{								   
										HANDLE h=ph.hp;//make_detach(ph.hp,h); 
								...
								refcount=ec_counter.AddRef();
								ipc_utils::DBG_PRINT("global ec ++refcount=%d", refcount);
							}
							else {
								refcount = ec_counter.Release();

								ipc_utils::DBG_PRINT("global ec --refcount=%d", refcount);


								if (refcount <= 0) {
									ipc_utils::DBG_PRINT("global ec signaled!!!");
									hevent.signal();
								}
							}
							...
							if(id==1)
							{
								...
								if(id==2)
								{
                                   ...
                                   return S_OK;
									...
									else if((id==id_msgwait))
									{
										while(!apartment_t<>::wait_loop(hevent.hko)){};
										return S_OK;
									}
									else if ((id == id_refcount))
									{
										if (res) {
											VARIANT v = { VT_I4 };
											ec_counter.AddRef();
											v.lVal = ec_counter.Release();
											*res = v;
										}										
										return S_OK;
									}
				...
				inline HRESULT impl_CreateInstanceEx(	IUnknown * pUnkOuter,LPOLESTR pszDisplayName,IStream* stream,REFIID riid,void ** ppvObject,IBindCtx * pbc)	
				{
					...
					if(global_mode&2)
					{
						...
						OLE_CHECK_hr(__super::impl_CreateInstanceEx(pUnkOuter,pszDisplayName,stream,__uuidof(IDispatch),disp._ppQI(),pbc));
					...
					else 	 hr=__super::impl_CreateInstanceEx(pUnkOuter,pszDisplayName,stream,riid,ppvObject,pbc);						 
			...
			{		
				...
				inline HRESULT impl_CreateInstanceEx(	IUnknown * pUnkOuter,LPOLESTR pszDisplayName,IStream* ,REFIID riid,void ** ppvObject,IBindCtx* pbc)	
				{
					...
					//size_t SZL = SysStringLen(pszDisplayName);
					
					moniker_parser_t<wchar_t> mp(pszDisplayName,pbc);
					char* tail;
					wchar_t* wproc=mp.trim(mp.bind_args(L"proc").def<bstr_t>());

					wchar_t* lib=mp.trim(mp.bind_args(L"lib").def<bstr_t>());
					if(!lib) lib=mp.trim(mp.bind_args(L"file").def<bstr_t>());
					wchar_t* path=mp.trim(mp.bind_args(L"path").def<bstr_t>());
					DWORD flags=mp.bind_args(L"flags").def<int>(0);
					DWORD flh=0;
					int ftmf=0;
					if((mp.bind_args(L"flags.mta").def<int>(0))) flh|=FLAG_CBD_MTA;
					if((mp.bind_args(L"flags.sta").def<int>(0))) flh|=FLAG_CBD_APP|FLAG_CBD_STA;
					if((ftmf=mp.bind_args(L"flags.fmt").def<int>(ftmf))) flh|=FLAG_CBD_FTM;
					if((mp.bind_args(L"flags.ftm").def<int>(ftmf))) flh|=FLAG_CBD_FTM;
					if((mp.bind_args(L"flags.git").def<int>(0))) flh|=FLAG_CBD_GIT;
					if((mp.bind_args(L"flags.ec").def<int>(0))) flh|=FLAG_CBD_EC;
					if((mp.bind_args(L"flags.old").def<int>(0))) flh|=FLAG_CBD_OLD;
					if((mp.bind_args(L"flags.com").def<int>(0))) flh|=FLAG_CBD_COM;
					if((mp.bind_args(L"flags.rev").def<int>(0))) flh|=FLAG_CBD_REVERSED;

					int fautodetect=mp.bind_args(L"flags.autodetect").def<int>(1);

					
					DWORD fm;

					fm=mp.bind_args(L"flags.mask").def<int>(DISPATCH_PUTSF);
					flags|= (flh<<16)|fm;
					...
					HRESULT hrs;
					hrs=HRESULT_FROM_WIN32(GetLastError());
					if(!hm) 
						if(fautodetect)
						{
							module_helper_t lib(fullibname);
							if(!lib.is_dll())
								 return hr=hrs;
							module_helper_t host;
							
							bool fh=host.is_win32();
							bool fl=lib.is_win32();
							if(!(fh^fl))
								return hr=hrs;
							bstr_t parsestr;

							if(fl) parsestr=L"ltx.bind:srv:32:dll:";
							else parsestr=L"ltx.bind:srv:64:dll:";


							argv_zzs<wchar_t> opts;
							opts[L"lib"]=fullibname;
							opts[L"proc"]=wproc;
							opts[L"flags"]=int(flags);
							opts[L"flags.ec"]=1;
							opts[L"flags.autodetect"]=0;

							parsestr+=opts.flat_str(false,L";");
							hr=ltxGetObjectEx(parsestr,riid,ppvObject,pbc);
							return hr;
				...
				:simple_factory_t<callback_factory_factory_t>					
			{
				...
				struct callback_dispatch_factory_t:base_aggregator_t<callback_dispatch_factory_t,ICallbackDispatchFactory>
				{
					...
					virtual HRESULT STDMETHODCALLTYPE bind_object(const wchar_t* moniker_str,REFIID riid,void** ppObj,IBindCtx* bctx=0){
						HRESULT hr;						
						return hr=::ltxGetObjectEx(moniker_str,riid,ppObj,bctx);
					};

					virtual HRESULT STDMETHODCALLTYPE bind_object_args(const wchar_t* moniker_str,REFIID riid,void** ppObj,int argc,VARIANT* argv){
						HRESULT hr;						
						return hr=::bindObjectWithArgsEx(moniker_str,riid,ppObj,false,argc,argv);
					};
					virtual HRESULT STDMETHODCALLTYPE bind_object_args_reverse(const wchar_t* moniker_str,REFIID riid,void** ppObj,int argc,VARIANT* argv){
						HRESULT hr;						
						return hr=::bindObjectWithArgsEx(moniker_str,riid,ppObj,true,argc,argv);
					};
				...
				inline HRESULT create_iaggregator(aggregator_container_helper_t& container,LPOLESTR pszDisplayName,IStream* ,GUID& uiid,smart_ptr_t<iaggregator_helper>& obj,IBindCtx * pbc )
				{
					//fs_disp=aggregator_helper_creator_t<fs_disp_t>::New(new fs_disp_t);
					//
					//mta_singleton_t::add_external_connector(container);
