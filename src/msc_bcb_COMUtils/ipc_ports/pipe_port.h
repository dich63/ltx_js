...
VOID GetAnswerToRequest(LPPIPEINST pipe)
{
       ...
       on_send_recv_(pcontext,++id,pipe->chRequest,pipe->cbRead,pipe->chReply,(int*)&(pipe->cbToWrite=0),hEvents[INSTANCES]);	
	/*
...
/*
std::pair<HANDLE,unsigned int> ppa= pipe_port<>::create_thread_instance(pn,on_send_recv,0,5000);
//
hea=ppa.first;

//hea=pipe_port<>::create_thread_instance(pn,on_send_recv,0,5000).hevent;
getchar();
printf("Abort\n");
pipe_port<>::shutdown(hea);
getchar();
*/
}
else
