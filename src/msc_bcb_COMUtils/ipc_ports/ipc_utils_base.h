...
#define SET_PROC_ADDRESS_name(h,a,name) ipc_utils::__set_dll_proc(h,(void**)&a,#name)
#define SET_PROC_ADDRESS_prfx(h,a,prfx) SET_PROC_ADDRESS_name(h,prfx##a,a)
#define SET_PROC_ADDRESS(h,a) SET_PROC_ADDRESS_name(h,a,a)
#define SET_PROC_ADDRESS_NS(h,ns,a) SET_PROC_ADDRESS_name(h,ns##::##a,a)
...
namespace ipc_utils {
	...
	inline static HRESULT CoGetLastError()
	{
	...
	inline void* __set_dll_proc(const void*  p,void** fp,char* name)
	{
	...
	struct   smart_ptr_t
	{
		...
		inline HRESULT detach(VARIANT* pv)
		{
             
			 if((!pv)||(!p)) return E_POINTER;
			 pv->vt=VT_UNKNOWN;
			 pv->punkVal=detach();
			return S_OK;
		}
	...
	struct clsid_t
	{
		...
		inline HRESULT QueryInterface( IUnknown* punk,void** ppOut){
			if(hr)
				return hr;
			if(!(ppOut&&punk))
				return E_POINTER;
			return punk->QueryInterface(clsid,ppOut);

		}

		inline HRESULT QueryInterface( IUnknown* punk){

			smart_ptr_t<IUnknown> t;			
			return QueryInterface(punk,t._ppQI());			
	...
	inline HRESULT  VARIANT2QueryInterface(VARIANT v, REFIID riid, void** ppObj) {
		...
		if (punk && ((vt == VT_DISPATCH) || (vt == VT_UNKNOWN))) {			
			hr = punk->QueryInterface(riid, ppObj);
		}
		return hr;
	}
...
*/
  ...
  static 	bool loop(HWND hwnd=0)
	...
	static 	DWORD _wait_alertable(HANDLE* ph, int nh = 1,int dwMilliseconds= INFINITE,DWORD dwFlags= MWMO_INPUTAVAILABLE | MWMO_ALERTABLE)
	{
		DWORD ws;
		ws = MsgWaitForMultipleObjectsEx(nh, ph, dwMilliseconds, QS_ALLINPUT, dwFlags);
		return ws;
	};
	...
	static int wait_loop(HANDLE* phko,int n=1,int dwMilliseconds = INFINITE, DWORD dwFlags = MWMO_INPUTAVAILABLE | MWMO_ALERTABLE)
	{
		...
		for (;;) {
			ws = _wait_alertable(phko, n, dwMilliseconds, dwFlags);
			
			bool f=(WAIT_TIMEOUT==ws);
			f = f || (WAIT_OBJECT_0 <= ws) && (ws < (WAIT_OBJECT_0 + n));
			f=f||( (WAIT_ABANDONED_0 <= ws) && (ws < (WAIT_ABANDONED_0 + n)) );
			f=f|| (ws == WAIT_FAILED);
			if (f)
				return ws;
			loop(0);
	...
	static 	bool sleep_msg(DWORD timeout)
	{
		DWORD ws;
		int tio=int(timeout);
		...
		{
			loop();
			ULONGLONG t=make_detach(t0,GetTickCount64());
			int dt=t0-t;
			tio-=dt;
			if(tio<=0) break;
		}
