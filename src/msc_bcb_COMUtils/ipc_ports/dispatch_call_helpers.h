...
namespace ipc_utils {
 ...
 inline   HRESULT operator =(variant_t v)
	...
	struct disp_call_t:invoker_base_t<disp_call_t,variant_t,HRESULT,VARIANT>
	{
		...
		inline 	res_t invoke_proc(int argc,argh_t* argv)
		{
			...
			{			
			 DISPPARAMS dps={argv,0,argc,0};
			 //WORD wFlags=(argc)? DISPATCH_METHOD:DISPATCH_METHOD|DISPATCH_PROPERTYGET;
			 WORD wFlags=(argc||(dispid))? DISPATCH_METHOD|DISPATCH_PROPERTYGET:DISPATCH_METHOD;
			 //if(dispid) wFlags=DISPATCH_METHOD|DISPATCH_PROPERTYGET;
			 res_t hr;
	...
	struct result_t
	{
		...
		template<class Intf>
		HRESULT QueryInterface(smart_ptr_t<Intf>& Obj)
		{
			HRESULT hr;
			return hr=QueryInterface(Obj.address()); 
		}
