...
namespace ipc_utils {
	namespace mbv2 {
		...
		struct  __declspec(uuid("{8F6714D1-8601-4e66-A8DD-7E37DD892B27}")) float64_class_t
		{
		...
		struct  __declspec(uuid("{710EC080-F1B1-4cc1-A6A4-F74DCD1006D3}")) uint8_class_t
		{
		...
		template <int VERS=0>
		struct lz4_shared_mem_t
		{
			//moniker_parser_t<>::bindObject()
		...
		struct shared_mem_t
		{
			...
			shared_mem_t(INT64 size=0):hfile(INVALID_HANDLE_VALUE),hmap(0),ptr(0),hr(E_FAIL),shdata(shared_mem_data_t()){
			...
			shared_mem_t& init (shared_mem_t& scr,INT64 _size,INT64 _offset,INT64 vt)
			{
		...
		struct mbv_buffer_ptr_aggregator_t:base_aggregator_t<mbv_buffer_ptr_aggregator_t<_ArrayTypeClass>,i_mbv_buffer_ptr>
		{
			...
			{
				pvt&&(*pvt=shmem->vt());
				pCount&&(*pCount=shmem->count());
				pElementSize&&(*pElementSize=shmem->element_size());
				return S_OK;
		...
		struct   mbv_buffer_t{
			...
			static 		HRESULT CreateContext(BOOL fUnmarshal,IBindCtx* bctx,LPOLESTR pszDisplayName,DWORD* pflags,void ** ppcontext){
				...
				if( pflags) * pflags=(1<< MSHCTX_INPROC)|0x10000; 
				int fm=fUnmarshal;
						...
						*/
						...
						else
						{
							...
							wchar_t* ptn=tn;
							 ptn=mn.trim(ptn);
							if(ptn)
								OLE_CHECK_hr(VARTYPE_finder_t<>::type_by_name(ptn,&vt));
			...
			static 		void InvokeByName(void* context,pcallback_context_arguments_t pcca,i_marshal_helper_t* helper){
				...
				else if((arguments.is_get_flag())&&(pm_name=arguments.pm_name()))
				{
					...
					if(moniker_parser_t<wchar_t>::_lcheckni(pm_name,L"close"))
					{
						psm->reset();
						return;
					}
			...
			static 		HRESULT MarshalToProcess(void* context,i_marshal_helper_t* helper,DWORD pid_target,IStream* pStm){
                ...
                OLE_CHECK_hr(helper->marshal_hko(pStm,psm->hfile));		
			...
			static 		HRESULT Unmarshal(void* context,i_marshal_helper_t* helper,IStream* pStm){
				...
				OLE_CHECK_hr(hr=helper->unmarshal_hko(pStm,&hfile));
