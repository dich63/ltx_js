	...
	//	struct  callback_class_factory_t:class_factory_base_aggregator_t<callback_class_factory_t<T,mcxmask> >
		...
		struct marshal_callback_t:marshal_base_aggregator_t<marshal_callback_t<_contextmask> >
		{
			...
			inline HRESULT IMarshal_MarshalInterface( IStream *pStm, REFIID riid, void *pv, DWORD dwDestContext, void *pvDestContext, DWORD mshlflags) {
				...
				{
					v_buf<wchar_t> buf;
					wchar_t*p=buf.printf(L"%s:__unmarshal__=-111\0\0",(wchar_t*)bstr_t(clsid));
					int cb=buf.size_b();
					memcpy(pmh->szz,p,cb);
					pmh->size+=cb;
			...
			//	
				...
				//co_releaser_t co_releaser(pStm);
				//hr=CoMarshalInterface(pStm,riid,cd.p,MSHCTX_LOCAL ,0,0*MSHLFLAGS_TABLESTRONG);

				//hr=CoUnmarshalInterface(pStm,__uuidof(IDispatch),callback._ppQI())
		...
		struct creator_class_base_t
		{
            ...
            template <class T>
			inline static HRESULT CreateInstance(T* p,LPOLESTR pszDisplayName,REFIID riid,void ** ppvObject,DWORD mcxmask=0,bool f_addIntf=true){
				
				HRESULT hr;
				return hr=p->getInstance(riid,ppvObject,0,mcxmask,f_addIntf);
...
/*
		...
		template <class I>
		struct creator_class2_t :creator_class_base_t
		{
...
#pragma pack(pop)
            inline static HRESULT marshal_BSTR(BSTR bstr,IStream* pStm)
			{
				HRESULT hr;
				ULONG len=SysStringByteLen(bstr),l;
				OLE_CHECK_hr(pStm->Write(&len,sizeof(len),&l));
				if(len) 
					OLE_CHECK_hr(pStm->Write(bstr,len,&l));
				return hr;
			}

			inline static HRESULT unmarshal_BSTR(IStream* pStm,BSTR* pbstr)
			{
				HRESULT hr;
				ULARGE_INTEGER l;
				LARGE_INTEGER len=LARGE_INTEGER();
				OLE_CHECK_hr(pStm->Read(&len.LowPart,sizeof(UINT),&l.LowPart));
				
				if(pbstr)
				{
					 BSTR b;					 
					if(b=SysAllocStringByteLen(0,len.LowPart))
					{
                       if(SUCCEEDED(hr=pStm->Read(b,len.LowPart,&l.LowPart)))
						   *pbstr=b;				   
					   else 
						   SysFreeString(b);
				...
				else hr=pStm->Seek(len,STREAM_SEEK_CUR,&l);
			...
			inline static HRESULT marshal_hko_to_process(HANDLE hko,DWORD pid_target,IStream* pStm)
			{
				...
				if(fsock=check_socket(hko))
				{
					...
					hsource_to_target.attrib=HANDLE64_t::attr_sock;
				...
				{
					    fsock=false;
						hsource_to_target.attrib=HANDLE64_t::attr_invalid;
				}	
			...
			inline static HRESULT unmarshal_hko(IStream* pStm,HANDLE* phko=0)
			{
				...
				if(HANDLE64_t::attr_invalid&hsource_to_target.attrib)
				{
					phko&&(*phko=INVALID_HANDLE_VALUE);
					return S_OK;
				}
       ...
       template<class T,class Intf,class _Creator=creator_class2_t<T> >
	   struct  base_callsback_marshal_t:base_marshal_utils_t<>
		{
			...
			{
				return base_marshal_utils_t<>::unmarshal_and_release(pStm,__uuidof(I),(void**)ppObj);
			}
			...
			inline HRESULT getInstance(REFIID riid,void ** ppvObject,IUnknown* p_aggregator_container=0,DWORD mcxmask=0,bool f_add_Intf=true)
			{
				...
				if(f_add_Intf) OLE_CHECK_hr(container.add(__uuidof(Intf),sO));
                   hr=container.add(__uuidof(IMarshal),sM).add(__uuidof(IPersistStream),sO).QueryInterface(riid,ppvObject);        
