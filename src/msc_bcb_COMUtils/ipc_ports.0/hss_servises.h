...
inline 	HRESULT transmit(SOCKET s,HANDLE hf,LONGLONG offset,LONG cb,LONG* pcbw,TRANSMIT_FILE_BUFFERS* ptfb=0)
	...
	struct http_srv_t
	{
		...
		http_srv_t(hss_servises_t* _owner,std::wstring _root):owner(_owner),root(_root){};
		http_srv_t(hss_servises_t* _owner):owner(_owner){};
...
{
	clear();
	std::vector<std::wstring> vipp,vhttpns,vhttpdir,vionsns;
	GUID gg={};
    idnull=str_uuid(gg);  
