...
namespace ipc_utils {
	...
	struct   smart_ptr_t
	{
		...
		{
			VARTYPE vt=(__uuidof(N)==__uuidof(IDispatch))?VT_DISPATCH:VT_UNKNOWN;
			VARIANT r={vt};
			r.byref=p;
			return r;
		}
	...
	inline bool VARIANT_is_Object(const VARIANT& v)
	{
        return (v.byref)&&((v.vt==VT_DISPATCH)||(v.vt==VT_UNKNOWN));
	}
