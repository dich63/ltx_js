			...
			struct ltx_js_server_t
			{
				...
				static	HRESULT console_holder(const wchar_t* lpszCmdLine)
				{
					...
					struct event_disp_t:IDispatch_base_t<event_disp_t,IDispatch,RefCounterFake_t<1> >
					{
						...
						inline	HRESULT impl_Invoke(DISPID id,REFIID,LCID,WORD,DISPPARAMS*,VARIANT*,EXCEPINFO*, UINT*)
					 {
						 if(id==0) {
							 hevent.signal();
							 return S_OK;
...

					{
						...
						*/
						...
						while(IsWindow(hwinc)&&(WAIT_TIMEOUT==(ws=WaitForSingleObject(hev,250)))){};
