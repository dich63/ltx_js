...
namespace ipc_utils {
	namespace com_apartments
	{
		...
		struct  __declspec(uuid("{5582D639-39B2-4015-85AD-632D3976B04C}")) IClassFactoryStream:IClassFactory{
			...
			virtual HRESULT __stdcall CreateInstanceEx(	IUnknown * pUnkOuter,LPOLESTR parse_string,IStream* stream ,REFIID riid,void ** ppvObject)=0;			
...
#pragma pack(pop)
		...
		struct class_factory_base_aggregator_t:base_aggregator_t<T,IClassFactoryStream>
		{
			...
			virtual HRESULT __stdcall CreateInstanceEx(	IUnknown * pUnkOuter,LPOLESTR parse_string,IStream* stream ,REFIID riid,void ** ppvObject)
			{
                      return static_cast<T*>(this)->impl_CreateInstanceEx(pUnkOuter,parse_string,stream,riid,ppvObject);
			}
			...
			inline HRESULT impl_ParseDisplayName(IBindCtx * pbc,LPOLESTR pszDisplayName,ULONG * pchEaten,IUnknown ** ppUnk)	{
				...
				return CreateInstanceEx(0,pszDisplayName,0,__uuidof(IUnknown),(void**)ppUnk);
			...
			inline HRESULT impl_CreateInstance(	IUnknown * pUnkOuter,REFIID riid,void ** ppvObject,void* params=0)	{
				return CreateInstanceEx(pUnkOuter,0,0,riid,ppvObject); 
			}
			inline HRESULT impl_CreateInstanceEx(	IUnknown * pUnkOuter,LPOLESTR parse_string,IStream* stream,REFIID riid,void ** ppvObject)	{return E_NOINTERFACE ;}
		...
		struct marshal_base_aggregator_t:base_aggregator_t<T,IMarshal>
		{
			...
			virtual HRESULT STDMETHODCALLTYPE GetMarshalSizeMax( 
				...
				/* [out] */ DWORD *pSize){
					...
					return static_cast<T*>(this)->IMarshal_GetMarshalSizeMax(riid,pv,dwDestContext,pvDestContext,mshlflags,pSize);
			};
			...
			virtual HRESULT STDMETHODCALLTYPE UnmarshalInterface( 
				...
				/* [out] */ void **ppv) {
					...
					return static_cast<T*>(this)->IMarshal_UnmarshalInterface(pStm,riid,ppv);
			};
