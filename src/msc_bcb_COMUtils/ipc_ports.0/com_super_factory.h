		...
		namespace script_engine
		{
				...
				:class_factory_base_aggregator_t<T>
			{
				...
				inline HRESULT impl_CreateInstanceEx(	IUnknown * pUnkOuter,LPOLESTR pszDisplayName,IStream* stream,REFIID riid,void ** ppvObject)	
				{
					...
					OLE_CHECK_hr(static_cast<T*>(this)->create_iaggregator(container,pszDisplayName,stream,uuid,helper));												
			...
			struct super_factory_t:class_factory_factory_t<super_factory_t>
			{
				...
				static HRESULT CoGetObject(bstr_t parse_string,REFIID riid,void** ppv)
				{
					OLE_CHECK_PTR(ppv);					
					...
					ULONG ec=0;
					OLE_CHECK_hr(pdn->ParseDisplayName(bctx,parse_string,&ec,&moniker.p));
				...
				struct binder_t:IDispatch_impl_t<binder_t>					
				{
					...
					inline	HRESULT impl_Invoke(DISPID id,REFIID riid,LCID,WORD flags,DISPPARAMS*dp,VARIANT*res,EXCEPINFO*pexi, UINT*){
						...
						try
						{
							...
							if( FAILED( hr=super_factory_t::CoGetObject(ps,__uuidof(IUnknown),unk._ppQI()) ) )
							{
