...
struct lh_marshal_t
{
	...
	*/
	...
	static		HRESULT marshal(int region_count,region_iterator iregion,ISequentialStream* stream,int pid=0,const uuid_t& conn_uuid=s_port_uuid(),COMCleaner_t* pCOMCleaner=0)
	{
      ...
      /*
		...
		COMCleaner_t localCOMCleaner;
		COMCleaner_t& COMCleaner=(pCOMCleaner&&pid)?(*pCOMCleaner):localCOMCleaner;
...
struct lh_marshal_map_t
{
	...
	inline HRESULT marshal(ISequentialStream* stream,int pid=0)
	{
		...
		vr[0]=&rm;
		region_t** pp=&vr[0];
		return hr=lh_marshal.marshal(region_count+1,pp,stream,pid,uuid(),&gCOMCleaner);
	}
	...
	v_buf<char*> names;
	DWORD tid;
	lh_marshal_t::COMCleaner_t gCOMCleaner;	
