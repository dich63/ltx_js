...
namespace ipc_utils {
	namespace com_apartments
	{
		...
		struct creator_class_t
		{
            ...
            template <class T>
			inline static HRESULT CreateInstance(T* t,LPOLESTR pszDisplayName,REFIID riid,void ** ppvObject,DWORD mcxmask=0)
			{
				HRESULT hr;

				return t->getInstance(riid,ppvObject,0,mcxmask);
				return hr;
       ...
       template<class T,class Intf,class _Creator=creator_class_t<__uuidof(T)> >
		struct  base_callsback_marshal_t
		{
			...
			inline  CLSID& get_clsid() const { 
				return creator_t::clsid;
			};
