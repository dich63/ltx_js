...
inline static HRESULT call_prop(void* proc,void*& pcntxt,DWORD dwFlags,int argc,VARIANT* pargv,VARIANT* presult,EXCEPINFO* pexcepinfo)
		{
	...
	struct  base_callsback_dispatch_t:base_callsback_dispatch_base_t
	{
		...
		inline	HRESULT impl_Invoke(DISPID id,REFIID riid,LCID,WORD flags,DISPPARAMS*dp,VARIANT*res,EXCEPINFO*pexi, UINT*){
			...
			if(!res) res=&tmp;
			   ...
			   hr=safe_caller_t<>::call_prop(pcalback,context,flagmask_hi|DWORD(flags),c,pdv,res,pexi);
