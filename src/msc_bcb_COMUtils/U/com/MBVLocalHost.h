...
struct external_connector
{
	...
	inline HRESULT out_unknown(ULONG ,VARIANT* )	{return E_NOTIMPL;}
	inline    HRESULT  unmarshal(VARIANT& v){  return E_NOTIMPL;}
	
};

template <class T> 
struct external_connector<T,true>
{
...
struct VariantWrapper
{
  ...
  VariantWrapper(VARIANT* pv=NULL,bool _fclear=true)
  {
	  ...
	  {
	       v=VARIANT();	  
		   if(!(v.parray=SafeArrayCreateVector(VT_UI1,0,sizeof(D)))) return;
...
template <class T>
class ATL_NO_VTABLE ISendReceiverImpl : 
	 public IEnumVARIANT
...
inline HRESULT ISendReceiver_Init()
{
	return S_OK;
}
...
inline  IUnknown* GetUnknown()
{
	return static_cast<IUnknown*>(this);
	 ...
	 STDMETHOD(QueryInterface)(REFIID riid, void** ppvObject)
	 {
		 //const bool faggr=(iid==GUID_NULL);
         HRESULT hr;   
		 if(!ppvObject) return E_POINTER;

		 if ((riid == IID_IUnknown)||(riid == __uuidof(IEnumVARIANT)))
			{
				*ppvObject = static_cast<void*>(this);
				AddRef();
				return S_OK;
			}
		 else if(m_pUnkExternalController)
         return hr=m_pUnkExternalController->QueryInterface(riid,ppvObject);
		  return E_NOINTERFACE;
	 }
	 ...
	 STDMETHOD(Next)(ULONG ,VARIANT*, ULONG*){return E_NOTIMPL;};
	 STDMETHOD(Skip)(ULONG ){return E_NOTIMPL;};
	 STDMETHOD(Reset)(void)
	 {	
		 HRESULT hr;
		 T* pT = static_cast<T*>(this);
		 return hr=pT->send_received();
	 }
	 STDMETHOD(Clone)(IEnumVARIANT**){return E_NOTIMPL;};

 HRESULT send_received(){return S_OK;}

template <class Intf>
static HRESULT CreateInstanseWithExternalCLSID(const CLSID& clsid,  Intf** ppUnk,const IID& iid=__uuidof(Intf))
{
	HRESULT hr;
    T* t=new T();
	hr=::CoCreateInstance(clsid,t->GetUnknown(),__uuidof(IUnknown),(void**)&(t->m_pUnkExternalController));
	if(FAILED(hr)) return delete t,hr;
    return hr=t->QueryInterface(iid,(void**)ppUnk);
}

ISendReceiverImpl():m_refcount(0){};
  
  LONG m_refcount;
  
  com_ptr<IUnknown> m_pUnkExternalController;
...
struct MARSHAL_DATA_declarator_N
{
	...
	struct MAP_DATA
	{
		union
		{
			struct{
				HANDLE hmap,hmutex;
				HANDLE hrsrvd[handle_count-1];
			};
			struct{HANDLE h[handle_count+1];};
		};
		...
		{ 
			...
			for(int n=0;n<=handle_count;n++) CloseHandle(h[n]);
		}

	};
	typedef MAP_DATA MARSHAL_DATA;
	MARSHAL_DATA m_mbv;
	...
	struct LockerPtr
	{
		...
		{
			hmutex=_this->get_MARSHAL_DATA().hmutex;
			if(!fnolock) 
				TryLock(INFINITE);
	...
	HRESULT IMarshalLocalHostData_MarshalToClientProcess(DWORD ClientProcessId,OUT MARSHAL_DATA& mcd) 
	{
