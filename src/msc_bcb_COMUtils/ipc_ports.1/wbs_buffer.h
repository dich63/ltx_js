...
struct wbs_op_t
{
	...
	HRESULT hr;
	H* hovl;
	wbs_op_t(H* _hovl):hovl(_hovl),hr(E_FAIL){};
...
{
	wbs_getter_t(HSS *h):wbs_op_t<wbs_getter_t<HSS>,HSS>(h){};
...
{
	wbs_setter_t(HSS *h):wbs_op_t<wbs_setter_t<HSS>,HSS>(h){};
...
struct wbs_frame_io_t
{
	...
	struct wbs_frame_t:wbs_frame_base_t
	{
	  ...
	  inline int64_t tail()
	  {
        ...
        template <class Getter> bool recv_header(Getter& sget,HRESULT& hr)
		{
			int cc,c;
			pos=0;
			len=0;
