...
inline static HRESULT call_prop(void* proc,void*& pcntxt,DWORD dwFlags,int argc,VARIANT* pargv,VARIANT* presult,EXCEPINFO* pexcepinfo,DISPID did)
		{
			...
			{&args_obj,&__raise_exception,pcntxt,argc,pargv,presult,wFlags,&hr,pexcepinfo,&pcntxt,0,&process_com_t::get_instance().callback_lib_arguments};
			cca.tss_id.id=did;
	...
	struct  base_callsback_dispatch_t:base_callsback_dispatch_base_t
	{
		...
		inline	HRESULT impl_Invoke(DISPID id,REFIID riid,LCID,WORD flags,DISPPARAMS*dp,VARIANT*res,EXCEPINFO*pexi, UINT*){
			...
			if(!res) res=&tmp;
			   ...
			   hr=safe_caller_t<>::call_prop(pcalback,context,flagmask_hi|DWORD(flags),c,pdv,res,pexi,id);
