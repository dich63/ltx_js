...
namespace ipc_utils {
	namespace com_apartments
	{
		...
		struct register_class_map_t:non_copyable_t
		{
			...
			inline HRESULT register_class( CLSID clsid,GetClassObject_t pClassObject,IUnknown** ppUnk,class_object_pair_t** pp_pair=0,IUnknown* marshal_context=0)
			{
				//smart_ptr_t<IUnknown> tmpunk;
				//if((!ppUnk)&&(!pp_pair))
				//	ppUnk=&tmpunk.p;
				...
				if(it==classes.end())
				{
					...
					*ppUnk=static_cast<IUnknown*>(cpn.p);
					...
					if(marshal_context)										
						(cpn->pmarshal_context=marshal_context)->AddRef();
				...
				else
				{
					if((pClassObject)&&(pClassObject!=(*it).p->GetClassObject))
						hr= E_ACCESSDENIED;
					else OLE_CHECK_hr((*it).p->QueryInterface(__uuidof(IUnknown),(void**)ppUnk));
					...
					if(marshal_context)										
					{
						class_object_pair_t *copt =static_cast<class_object_pair_t*>((*it).p);
						smart_ptr_t<IUnknown> unk(copt->pmarshal_context,false);
						(copt->pmarshal_context=marshal_context)->AddRef();
					}
			...
			inline HRESULT get_class( CLSID clsid,IUnknown** ppUnk,class_object_pair_t** pp_pair=0)
			{
				return register_class(clsid,0,ppUnk,pp_pair);
            			...
            			inline HRESULT register_marshal_context( CLSID clsid,IUnknown* marshal_context)
						{
							   OLE_CHECK_PTR(marshal_context);
                               HRESULT hr;
							   locker_t<register_class_map_t> lock(this);
							   smart_ptr_t<IUnknown> unk;
							   pclass_object_pair_t pc=0;
							   OLE_CHECK_hr(register_class(clsid,0,unk._address(),&pc));
							   smart_ptr_t<IUnknown> umc(pc->pmarshal_context,false);
							   umc.reset(marshal_context);
							   pc->pmarshal_context=umc.detach();
							   return hr;							   
						}
            ...
            static HRESULT GetClassObject(REFCLSID rclsid, REFIID riid, LPVOID* ppv)
			{
				...
				locker_t<register_class_map_t> lock(rmap);
				smart_ptr_t<IUnknown> unk;
			...
			static HRESULT GetClassObject(const wchar_t* pmoniker, REFIID riid, void** ppv, IUnknown** ppmarshal_context=0)
			{
				//OLE_CHECK_PTR(ppv);
				HRESULT hr;
				register_class_map_t& rmap=get_instance();
				...
				ppmarshal_context&&ppair->pmarshal_context&&(*ppmarshal_context=ppair->pmarshal_context)&&(ppair->pmarshal_context->AddRef());
				if(ppv) OLE_CHECK_hr(hr=ppair->GetClassObject(ppair->clsid,riid,ppv));						
				return hr;
					...
					OLE_CHECK_hr(hr);
						...
						else if(header->is_bind())
						{
							...
							if(pend=moniker_parser_t<wchar_t>::_lskipi(p,L":",-1))
							{
							 ...
							 OLE_CHECK_hr(register_class_map_t<>::GetClassObject(ws.c_str(),__uuidof(IClassFactory),cf._ppQI()));//,marshal_context._address()));
							 smart_ptr_t<IClassFactoryStream> cfs;
							 if(SUCCEEDED(cf.QueryInterface(&cfs.p)))
							 {
								 //if(marshal_context)									 OLE_CHECK_hr(cfs->SetMarshalContext(marshal_context));

                               OLE_CHECK_hr(cfs->CreateInstanceEx(0,p,pStm,riid,ppv,0));
							   return hr;
