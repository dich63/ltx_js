...
static void ctmf_helper(
		...
		,uint16_t *h_coarse,uint16_t *h_fine
        )
{
...
static void ctmf(
        ...
        const int width, const int height,
        const int src_step, const int dst_step,
        const int r, const int cn, const long unsigned int memsize=(512*1024)
        )
{
    /*
	...
	h_coarse = (uint16_t*) _mm_malloc(  1 * 16 * width * cn * sizeof(uint16_t), 16 );
	h_fine   = (uint16_t*) _mm_malloc( 16 * 16 * width * cn * sizeof(uint16_t), 16 );
    ...
    for ( i = 0; i < width; i += stripe_size - 2*r ) {
        ...
        ctmf_helper( src + cn*i, dst + cn*i, stripe, height, src_step, dst_step, r, cn,
                i == 0, stripe == width - i,h_coarse,h_fine );
...
static void ctmf_mt(const int begin_row,const int end_row,
        const unsigned char* const src, unsigned char* const dst,
        const int width, const int height,
        const int src_step, const int dst_step,
        const int r, const int cn, const long unsigned int memsize=(512*1024)
        )
{
    ...
    for ( i = begin_row; i <= end_row; i += stripe_size - 2*r ) {
        int stripe = stripe_size;
        /* Make sure that the filter kernel fits into one stripe. */
        if ( i + stripe_size - 2*r >= width || width - (i + stripe_size - 2*r) < 2*r+1 ) {
            stripe = width - i;
