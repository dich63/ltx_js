...
typedef unsigned __int32 stride_t;
...
long WriteBMP(wchar_t *name, int sx, int sy, int bpp, const void *pData)
{
	FILE *F = NULL;

	if((F = _wfopen(name, L"wb")) == NULL )
	  return E_FAIL;

	BITMAPINFO DIB;

	DIB.bmiHeader.biSize          = sizeof(DIB.bmiHeader);
	DIB.bmiHeader.biWidth         = sx;
	DIB.bmiHeader.biHeight        = sy;
	DIB.bmiHeader.biPlanes        = 1;
	DIB.bmiHeader.biBitCount      = (short)bpp;
	DIB.bmiHeader.biCompression   = 0;//compression;
	DIB.bmiHeader.biSizeImage     = (((sx * bpp >> 3) + 3) & ~3) * sy;
	...
	FILEHDR.bfOffBits   = sizeof(FILEHDR)   + DIB.bmiHeader.biSize;
	FILEHDR.bfSize      = FILEHDR.bfOffBits + DIB.bmiHeader.biSizeImage;
	FILEHDR.bfReserved1 =
	FILEHDR.bfReserved2 = 0;

	if(
	   fwrite(&FILEHDR,        1, sizeof(FILEHDR),          F) != sizeof(FILEHDR) ||
	   fwrite(&DIB.bmiHeader,  1, sizeof(DIB.bmiHeader),    F) != sizeof(DIB.bmiHeader) ||
	   fwrite(pData,           1, DIB.bmiHeader.biSizeImage,F) != DIB.bmiHeader.biSizeImage)
	  return E_FAIL;
